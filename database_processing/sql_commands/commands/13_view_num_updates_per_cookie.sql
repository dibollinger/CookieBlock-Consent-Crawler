...
-- requires taining data view
CREATE VIEW IF NOT EXISTS view_num_updates_per_cookie AS
SELECT visit_id, site_url, name, actual_domain, path, COUNT(training_data.visit_id) as num_diffs
FROM
(
...
WHERE j.record_type <> "deleted" and c.cat_id <> -1
ORDER BY j.visit_id, name, j.time_stamp ASC
) as training_data
GROUP BY visit_id, name, actual_domain, path
