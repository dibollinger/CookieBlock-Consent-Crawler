...
/*
 ...
 # Stores data collected from consent frames, such as cookie notices or consent walls.
 */
...
CREATE TABLE IF NOT EXISTS consent_data (
   ...
   browser_id INTEGER NOT NULL,             -- crawl id where this record was collected
   visit_id INTEGER NOT NULL,               -- visit id for the site
   name TEXT NOT NULL,                      -- name as specified in the cookie consent platform
   domain TEXT NOT NULL,                    -- domain as specified in the cookie consent platform
   ...
   expiry TEXT,                             -- some CMPs define an expiration time
   type_name VARCHAR(256),                  -- Cookiebot describes different cookie "types". Some cookies are set by javascript, some are set over HTTP. Others, such as pixels, aren't cookies at all.
   type_id INTEGER                          -- ID of the above type
);

/*
 # Consent Crawl Status Table
 # Stores the crawl results of a crawl, including details on success or failure.
 */

CREATE TABLE IF NOT EXISTS consent_crawl_results (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    browser_id INTEGER NOT NULL,
    visit_id INTEGER NOT NULL,
    cmp_type INTEGER NOT NULL,
    crawl_state INTEGER NOT NULL,
    report TEXT
)