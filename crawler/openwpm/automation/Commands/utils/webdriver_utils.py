...
# Altered from its original release
...
def wait_until_loaded(webdriver: WebDriver, timeout:float,
                      period:float=0.25, min_time:float=0) -> bool:
    """
    Sleeps until the website is fully loaded, or until timeout is up.
    Sleeps a minimum of specified time, and checks every period seconds.
    """
...
# ####### Search Helpers ########
def wait_and_find(driver: WebDriver, locator_type: str, locator: str,
                  timeout: int = 3, check_iframes: bool = True):
...
def is_found(driver: WebDriver, locator_type:str, locator:str, timeout:int=3):
    """
    Return true if the specified locator is found.
    False otherwise or on exception.
    """
    try:
...
def is_visible(driver: WebDriver, locator_type:str, locator:str, timeout:int=3):
    """
    Return true if an element with the specified locator is visible.
    False otherwise or on exception.
    """
    try:
        w = WebDriverWait(driver, timeout)
...
def title_is(driver: WebDriver, title:str, timeout:int=3):
    """
    Compare exact match of the title,  wait until match or timeout occurs.
    """
    try:
        w = WebDriverWait(driver, timeout)
...
def title_contains(driver: WebDriver, title:str, timeout:int=3):
    """Check if title contains specified string, wait until match or timeout occurs."""
    try:
        w = WebDriverWait(driver, timeout)
...
def is_clickable(driver: WebDriver, full_xpath:str, xpath:str, timeout:int=1):
...
def click_to_element(element: WebElement, sleep_after:float=0.5):
    """Click the provided element and handle WebDriverException."""
    try:
        element.click()
...
def move_to_element(driver: WebDriver, element: WebElement):
    """ Move the cursor over the provided element, handle Webdriver Exceptions."""
    try:
...
def move_to_and_click(driver: WebDriver, element: WebElement, sleep_after: float = 0.5):
...
def is_displayed(element: WebElement):
    """ Check if the specified element is being displayed. Return False also on exception."""
    try:
...
def is_active(input_element: WebElement):
...
def get_button_text(element: WebElement):
...
def iter_frames(driver: WebDriver):
...
def switch_to_parent_frame(driver: WebDriver, frame_stack: List[WebElement]):
...
def execute_in_all_frames(driver: WebDriver, func: Any, kwargs: Any={},
                          frame_stack:List[WebElement]=['default'],
                          max_depth:int =5, logger: Any=None, visit_id:int=-1):
