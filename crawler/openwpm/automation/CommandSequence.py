...
from .Commands.Types import (BaseCommand, BrowseCommand, DumpPageSourceCommand,
                             ...
                             ScreenshotFullPageCommand, CookiebotScrapeCommand,
                             OneTrustScrapeCommand, TermlyScrapeCommand,
                             GenericCMPScrapeCommand)
...
class CommandSequence:
    ...
    def browse(self, num_links=2, sleep=0, timeout=60, subpage_timeout=10):
        """ browse a website and visit <num_links> links on the page """
        self.total_timeout += timeout
        command = BrowseCommand(self.url, num_links, sleep, subpage_timeout)
        self._commands_with_timeout.append((command, timeout))
        self.contains_get_or_browse = True
    ...
    def run_consent_crawl(self, num_links=2, sleep=1.0, timeout=300,
                          abort_browse_early=True, subpage_timeout=10.0):
        """
        Performs a complete consent management platform crawl, looking for all CMPs that are supported.
        If the CMP crawl succeeded, then subsequently performs a browse command.
        If the CMP crawl failed, will abort the command (without an interrupt).
        @param num_links: Number of subpages to browse as part of the browse command.
        @param sleep: Amount of seconds to sleep after the GET request.
        @param timeout: Timeout until the command aborts automatically.
        @param abort_browse_early: If True, will abort browse part if CMP command failed.
        @param subpage_timeout: How long to wait at most for each subpage to load.
        """
        self.total_timeout += timeout
        command = GenericCMPScrapeCommand(self.url, num_links, sleep, abort_browse_early, subpage_timeout)
        self._commands_with_timeout.append((command, timeout))
        self.contains_get_or_browse = True


    def try_extract_cookiebot_data(self, sleep=0, timeout=60):
        """
        Send a GET request to the URL using the webdriver and then search
        for data pertaining to the Cookiebot Consent Management Provider.
        @param sleep: Amount of seconds to sleep after the GET request.
        @param timeout: Amount of time in seconds until the command is aborted.
        """
        self.total_timeout += timeout
        command = CookiebotScrapeCommand(self.url, sleep)
        self._commands_with_timeout.append((command, timeout))
        self.contains_get_or_browse = True


    def try_extract_onetrust_data(self, sleep=0, timeout=60):
        """
        Send a GET request to the URL using the webdriver and then search
        for data pertaining to the OneTrust Consent Management Provider.
        @param sleep: Amount of seconds to sleep after the GET request.
        @param timeout: Amount of time in seconds until the command is aborted.
        """
        self.total_timeout += timeout
        command = OneTrustScrapeCommand(self.url, sleep)
        self._commands_with_timeout.append((command, timeout))
        self.contains_get_or_browse = True


    def try_extract_termly_data(self, sleep=0, timeout=60):
        """
        Send a GET request to the URL using the webdriver and then search
        for cookie category data of the Termly Consent Management Provider.
        @param sleep: Amount of seconds to sleep after the GET request.
        @param timeout: Amount of time in seconds until the command is aborted.
        """
        self.total_timeout += timeout
        command = TermlyScrapeCommand(self.url, sleep)
        self._commands_with_timeout.append((command, timeout))
        self.contains_get_or_browse = True
